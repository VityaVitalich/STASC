...
def main():
    ...
    initial_generation_few_shot_prompts = load_few_shot_prompts(
        config["few_shot_dir"], "debate_generation"
    )
    ...
    prompt_builder = DebatePromptBuilder(config)
    ...
    test_data = perform_generation(
        ...
        sampling_params=sampling_params,
        id_key=config["id_col"],
        output_col="judgement",
    )

    revised_acc = KM(
        test_data, target_col="judgement", gt_col=config["gold_col"], evaluator=reward_function
    )
